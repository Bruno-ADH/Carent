<section class="space-y-6">
    ...
    <x-danger-button
        x-data=""
        x-on:click.prevent="$dispatch('open-modal', 'confirm-user-deletion')" style="background-color: #FD1414"
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
            ...
            <div class="mt-6 flex justify-end">
                <x-secondary-button x-on:click="$dispatch('close')" style="background-color: #040F28">
                    {{ __('Annuler') }}
                </x-secondary-button>

                <x-danger-button class="ms-3" style="background-color: #FD1414">
                    {{ __('Supprimer le compte') }}
                </x-danger-button>
            </div>
        </form>
